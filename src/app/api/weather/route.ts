...
export async function GET() {
  ...
  if (mode === "mock") {
    ...
    return NextResponse.json({
      ...
      current: {
        ...
        humidity: 65,
        windSpeed: 24,
        rainProb: (fused as any)?.rainProb || 82,
        updatedAt: new Date().toISOString(),
        sources: (fused as any)?.sources,
        confidence: (fused as any)?.confidence,
      },
      risks,
  ...
  return NextResponse.json(
    {
      error:
        "Live mode not configured — set WEATHER_API_KEY and DATA_MODE=live",
    },
    { status: 503 },
  );
}
