...
const GOOGLE_KEY =
  ...
  "AIzaSyAC5Z1irvXGd16Yoyf8gYg9yjjKHBphW-w";
...
export default function MapPage() {
  ...
  useEffect(() => {
    ...
    if (existing) {
      ...
      existing.addEventListener("error", () => {
        setStatus("error");
        setErrorMsg(
          "Google Maps script failed to load — check key, billing, and HTTP referrer restrictions.",
        );
      });
      return;
    }
    ...
    s.onerror = () => {
      setStatus("error");
      setErrorMsg(
        "Failed to load Google Maps. Common causes: 1) Billing not enabled 2) Maps JavaScript API not enabled 3) HTTP referrer blocked. Check Google Console → Credentials → your key → Application restrictions.",
      );
    };
    ...
    function initMap() {
      ...
      try {
        ...
        (window as any).gm_authFailure = () => {
          setStatus("error");
          setErrorMsg(
            "Google Maps authentication failed: Invalid key or HTTP referrer not allowed. Go to Google Console → APIs & Services → Credentials → Edit your key → Add http://localhost:3000/* to HTTP referrers, and enable Maps JavaScript API + Geocoding API. Also enable Billing.",
          );
        };
        renderOverlays(map);
      } catch (e: any) {
  ...
  function renderOverlays(map: any) {
    ...
    if (
      activeLayers.includes("Government warnings") ||
      activeLayers.includes("Rain") ||
      true
    ) {
      const marker = new g.Marker({
        position: { lat: 11.6643, lng: 78.146 },
        map,
      ...
      const info = new g.InfoWindow({
        content: `<div style="font:12px sans-serif"><b>🔴 Salem, TN</b><br/>31°C • Humidity 65%<br/>Heavy Rain 85% • CRITICAL<br/>Coastal flood risk next 3h</div>`,
      });
    ...
    if (activeLayers.includes("User reports")) {
      mockReports.forEach((r) => {
        ...
        const info = new g.InfoWindow({
          content: `<div style="font:11px sans-serif"><b>${r.text}</b><br/>${r.status} • ${r.time} • ${r.user}</div>`,
        });
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col pb-16">
      <header className="bg-white border-b px-4 py-3 flex items-center justify-between sticky top-0 z-30">
        <div className="font-bold text-[#0f2942]">
          🗺️ Live Weather Map{" "}
          <span className="text-[10px] font-normal text-slate-400 ml-2 hidden md:inline">
            Google Maps • {GOOGLE_KEY.slice(0, 12)}...
          </span>
        </div>
      ...
      <div className="flex-1 grid lg:grid-cols-4 gap-0">
        <div className="lg:col-span-3 relative bg-slate-100 h-[55vh] lg:h-[calc(100vh-112px)] overflow-hidden">
            ...
            <div className="absolute inset-0 bg-white/90 backdrop-blur p-4 overflow-auto">
              <div className="max-w-lg mx-auto bg-red-50 border border-red-200 rounded-2xl p-4">
                ...
                <div className="text-xs bg-white border rounded-xl p-3 mt-3 leading-relaxed">
                  ...
                  <br />
                  2. Click your key{" "}
                  <code>AIzaSyAC5Z1irvXGd16Yoyf8gYg9yjjKHBphW-w</code>
                  <br />
                  3. Application restrictions → <b>Website restrictions</b> →
                  Add:
                  <br />
                  ...
                  <br />
                  4. API restrictions → Enable <b>Maps JavaScript API</b> +{" "}
                  <b>Geocoding API</b>
                  <br />
                  5. <b>Billing</b> must be enabled (even free tier).
                  <br />
                ...
                <div className="text-[11px] text-slate-400 mt-2">
                  Fallback mock shown below if Google is blocked. Check browser
                  console (F12) for{" "}
                  <code>Google Maps API error: RefererNotAllowedMapError</code>.
                </div>
              </div>
            </div>
          )}
          ...
          <div className="absolute bottom-3 left-3 right-3 bg-white rounded-2xl shadow-lg border p-3 flex items-center gap-3">
            ...
            <input
              ...
              max={100}
              defaultValue={30}
              className="flex-1 accent-[#0f2942]"
            />
        ...
        <div className="bg-white border-t lg:border-t-0 lg:border-l p-4 space-y-4 overflow-auto">
          <div>
            ...
            <div className="text-[11px] text-slate-400 mt-2">
              Toggle layers updates Google Maps markers/circles live
            </div>
          </div>
              ...
              {mockReports.slice(0, 3).map((r) => (
                <div key={r.id} className="border rounded-xl p-2.5">
                  ...
                  <div className="text-[11px] text-slate-500 mt-1 flex justify-between">
                    ...
                    <span
                      className={`px-1.5 py-0.5 rounded-full text-[10px] ${r.status === "Verified" ? "bg-emerald-100 text-emerald-700" : r.status === "Corroborated" ? "bg-amber-100 text-amber-700" : "bg-slate-100"}`}
                    >
                      {r.status}
                    </span>
                  </div>
          ...
          <div className="bg-slate-50 border rounded-xl p-2 text-[11px] text-slate-500">
            Key loaded from <code>NEXT_PUBLIC_GOOGLE_MAPS_API_KEY</code>. If you
            see <code>RefererNotAllowedMapError</code>, add{" "}
            <code>http://localhost:3000/*</code> in Google Console → Credentials
            → HTTP referrers. Ensure Billing is enabled.
          </div>
        </div>
      </div>
      <Nav />
