...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const analyzed = analyzeRisks({
      rainIntensity: mockCurrent.rainProb,
      rainDurationH: 5,
      windSpeed: mockCurrent.windSpeed,
      visibility: mockCurrent.visibility,
      temp: mockCurrent.temp,
      terrainRisk: "medium",
    ...
    const id = setInterval(
      () => setLastUpdated(`${Math.floor(Math.random() * 5) + 1} min ago`),
      30000,
    );
    ...
    <div className="min-h-screen bg-slate-50 pb-16 md:pb-0">
      <header className="sticky top-0 z-40 bg-white border-b px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link
            href="/settings"
            className="w-8 h-8 rounded-full bg-slate-200 grid place-items-center text-sm"
          >
            👤
          </Link>
      ...
      <main className="max-w-6xl mx-auto px-3 md:px-6 py-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-2xl p-4 border flex items-center justify-between">
            <div>
              <div className="font-bold text-[#0f2942]">
                {locationName} • {weather.temp}°C | Humidity {weather.humidity}%
              </div>
              <div className="text-xs text-slate-500">
                {weather.condition} •{" "}
                {profile?.location?.lat?.toFixed(4) || "11.6643"},{" "}
                {profile?.location?.lon?.toFixed(4) || "78.1460"} • Feels like{" "}
                {weather.feelsLike}°C
              </div>
            </div>
            <Link
              href="/map"
              className="text-xs bg-sky-50 text-sky-700 px-3 py-1.5 rounded-full border border-sky-200"
            >
              View Map →
            </Link>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-white rounded-2xl p-4 border">
              ...
              <div className="mt-2 flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-500">
                    Feels like {weather.feelsLike}°C • Wind {weather.windSpeed}{" "}
              ...
              <div className="mt-3 text-[11px] bg-slate-50 rounded-xl p-2 border">
                ...
                <div className="text-slate-600">
                  Weather Updated: {lastUpdated} • Confidence:{" "}
                  <span className="font-semibold text-emerald-600">
                    {fused?.confidence || weather.confidence}
                  </span>
                </div>
                <div className="flex gap-2 mt-1 flex-wrap">
                  {(fused?.sources || weather.sources).map((s: string) => (
                    <span
                      key={s}
                      className="px-1.5 py-0.5 bg-white border rounded-full text-[10px]"
                    >
                      ✓ {s}
                    </span>
                  ))}
                ...
                {fused && (
                  <div className="text-[10px] text-slate-400 mt-1">
                    Fused from {fused.raw.length} sources • Variance &lt;10% →
                    High confidence
                  </div>
                )}
              </div>
            ...
            <div className="bg-gradient-to-br from-sky-500 via-sky-500 to-emerald-500 text-white rounded-2xl p-4 flex flex-col">
              <div className="text-xs font-semibold opacity-90">
                WeatherGPT Advice For You
              </div>
              <div className="text-[11px] opacity-80">
                Occupation: {occupation} • Location: {locationName}
              </div>
              <p className="text-sm mt-3 leading-relaxed">"{advice}"</p>
              <div className="mt-3 grid grid-cols-3 gap-2 text-center text-xs">
                <div className="bg-white/20 backdrop-blur rounded-xl p-2">
                  <div className="opacity-80">RAIN PROB</div>
                  <div className="font-bold text-base">{weather.rainProb}%</div>
                </div>
                <div className="bg-white/20 backdrop-blur rounded-xl p-2">
                  <div className="opacity-80">EXPECTED</div>
                  <div className="font-bold">12mm</div>
                </div>
                <div className="bg-white/20 backdrop-blur rounded-xl p-2">
              ...
              <div className="mt-3 flex gap-2">
                <Link
                  href="/chat"
                  className="flex-1 bg-white text-[#0f2942] rounded-full py-2 text-center text-sm font-semibold"
                >
                  Ask WeatherGPT
                </Link>
                <button
                  onClick={() => {
                    if ("speechSynthesis" in window) {
                      ...
                      u.lang =
                        profile?.primaryLang === "ta" ? "ta-IN" : "en-US";
                      speechSynthesis.speak(u);
                    }
                  }}
                  className="px-3 py-2 bg-white/20 backdrop-blur rounded-full text-sm"
                >
                  🔊
                </button>
              </div>
              <div className="text-[10px] opacity-70 mt-2">
                Same verified forecast → personalized action. LLM explains,
                never fabricates.
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 border">
            ...
            <div className="mt-2 divide-y">
              {mockDaily.map((d) => (
                ...
                >
                  <span className="w-16">{d.day}</span>
                  <span>{d.icon}</span>
                  <span className="text-sky-600 text-xs">{d.rainProb}%</span>
        ...
        <div className="space-y-4">
          {showCritical && (
            <div className="bg-gradient-to-b from-red-700 to-red-500 text-white rounded-2xl p-4 border border-red-800">
              <div className="flex items-center justify-between">
                <span className="text-xs bg-white text-red-700 px-2 py-1 rounded-full font-bold">
              ...
              <p className="text-xs mt-3 font-mono opacity-90">
                உங்கள் பகுதியில் கனமழை மற்றும் வெள்ள அபாயம் உள்ளது. தேவையற்ற
                பயணங்களைத் தவிர்க்கவும்.
              </p>
              <p className="text-sm mt-2">
                Heavy rainfall and flood risk are expected. Avoid unnecessary
                travel.
              </p>
              <div className="mt-3 bg-white text-slate-900 rounded-xl p-2.5 flex justify-between text-xs">
                <span>📍 {profile?.location?.name || "Coastal Regions"}</span>
                <span>Next 3 Hours</span>
              </div>
              <div className="mt-2 flex flex-col gap-2">
                <Link
                  href="/emergency"
                  className="bg-white text-red-700 rounded-xl py-2 text-center text-sm font-semibold"
                >
                  📍 Find Safe Route
                </Link>
                <Link
                  href="/emergency"
                  className="bg-white/20 backdrop-blur rounded-xl py-2 text-center text-sm"
                >
                  🏠 Nearest Shelter
                </Link>
                <button
                  onClick={() => {
                    if ("speechSynthesis" in window) {
                      const u = new SpeechSynthesisUtterance(
                        "Heavy rainfall and flood risk are expected. Avoid unnecessary travel.",
                      );
                      speechSynthesis.speak(u);
                    }
                  }}
                  className="bg-white text-slate-900 rounded-xl py-2 text-sm"
                >
                  🔊 Playing Voice Alert…
                </button>
              </div>
          ...
          <div className="bg-white rounded-2xl p-4 border">
            ...
            <div className="text-xs text-slate-500">
              Calculated by Risk Engine • Not raw API claims
            </div>
            <div className="mt-3 space-y-2">
              {mockRisks.map((r) => (
                ...
                >
                  <div>
                    <div className="text-sm font-semibold">{r.hazard}</div>
                    <div className="text-xs text-slate-500">
                      {r.location} • {r.time}
                    </div>
                  </div>
                  <span
                    className={`text-xs px-2 py-1 rounded-full font-bold ${r.level === "Critical" ? "bg-red-600 text-white" : r.level === "High" ? "bg-orange-500 text-white" : r.level === "Moderate" ? "bg-amber-400 text-slate-900" : "bg-emerald-100 text-emerald-700"}`}
                  >
                    {r.level}
                  </span>
                </div>
          ...
          <div className="bg-white rounded-2xl p-4 border">
            ...
            <div className="text-xs text-slate-500">
              Good Morning, {profile?.name || "Arun"} 👋
            </div>
            <div className="mt-2 grid grid-cols-3 gap-2 text-center text-xs">
              <div className="bg-slate-50 rounded-xl p-2">🌡️ 31°C</div>
              <div className="bg-slate-50 rounded-xl p-2">🌧️ 72%</div>
              <div className="bg-slate-50 rounded-xl p-2">💨 24 km/h</div>
            </div>
            <p className="text-sm mt-3 bg-sky-50 border border-sky-100 rounded-xl p-3">
              "Rain is likely after 4 PM. Consider completing outdoor work
              earlier."
            </p>
            <button
              onClick={() => {
                if ("speechSynthesis" in window) {
                  const u = new SpeechSynthesisUtterance(
                    "Good morning. Rain is likely after 4 PM. Consider completing outdoor work earlier.",
                  );
                  speechSynthesis.speak(u);
                }
              }}
          ...
          <div className="bg-[#0f2942] text-white rounded-2xl p-4">
            ...
            <div className="grid grid-cols-2 gap-2 mt-3 text-sm">
              <Link
                href="/community"
                className="bg-white text-[#0f2942] rounded-xl py-2 text-center"
              >
                📸 Report Conditions
              </Link>
              <Link
                href="/chat"
                className="bg-white/20 backdrop-blur rounded-xl py-2 text-center"
              >
                💬 Ask WeatherGPT
              </Link>
              <Link
